...
6. 单个团队盘限制 40 W 文件数，注意不要超限。
    ...
    2. 跑完一遍 `sync` 后使用 `rclone size` 分别查看【源盘】和【目的盘】的文件数和大小（`size` 命令使用 gclone 会有点 bug ，所以此处切换回 Rclone），如果【源盘】大于目标盘，说明没复制完全，多跑几次 `sync` 命令后再比较，直到【目的盘】大于【源盘】，说明有重复文件了，此时使用 `dedup` 命令对【目的盘】去重；
    
    3. 再次 `size` 查看文件数，如果【源盘】文件数大于【目的盘】，就重复 `sync` ，如果【源盘】小于【目的盘】，就使用 `dedup` 去重；
    
    4. 重复以上步骤，直到 `rclone size` 后【源盘】和【目的盘】的文件数和大小严格相等。

    这个问题多出现在【源盘】是个人盘的共享文件夹的时候，因为个人盘的 API 有点 bug ，会导致分享的文件数过多时，通过 API 操作时读取到的文件列表不全，需要等待一段时间才能读取完全，团队盘一般不会出现这个问题。
