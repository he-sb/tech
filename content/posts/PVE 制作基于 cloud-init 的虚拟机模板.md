...
导入完成后，查看虚拟机的【硬件】配置，应该能看到下方多了一行 `未使用的磁盘 0`，双击它，勾选 `SSD 仿真` （这一步非必需），`总线 / 设备` 选择 `SCSI` ，点击“添加”按钮，添加为磁盘后，单击新添加的磁盘，点击上方的“磁盘操作”按钮，选择“调整大小”，输入 `1` ，点击“确认”，给磁盘增加 1 G容量，否则下文配置时会出现磁盘空间不足的情况（默认的硬盘只有 2 G 容量）。
