...
> 特别地，Debian 系发行版使用的包管理工具 `apt` 是不使用上面的环境变量的，简单的解决方式是添加形如 `-o Acquire::http::proxy='socks5h://192.168.1.111:1080/'` 的参数来指定代理，比如检查更新时使用
> 
> ```shell
> sudo apt -o Acquire::http::proxy='socks5h://192.168.1.111:1080/' update
> ```
> 
> 如果需要默认走代理，那么需要修改 apt 的配置文件，这部分内容请自行谷歌。
